...
float h = stepSize;
    ...
    vector<Particle *> midstate;
    vector<Particle *> newstate;

    for (unsigned i = 0; i < state.size(); ++i) {
		Vector3f newVec = state[i]->position+ h*f0[2*i];                                               
		//cout << "F0 number 0: " << f0[0][0] <<" " << f0[0][1] <<" " <<  f0[0][2] << endl;
		//cout << "Gravity: " << f0[1][0] << " "<<  f0[1][1] << " " << f0[1][2] << endl;
		Particle * p1 = new Particle(state[i]->mass);
		p1->position = newVec;
		p1->velocity = state[i]->velocity+h*f0[2*i+1];    
		...
		midstate.push_back(p1);
    }

    vector<Vector3f> f1 = particleSystem -> evalF(midstate);

	for (unsigned j = 0; j < state.size(); ++j) {
		Vector3f newVec1 = state[j]->position+ h/2.*(f0[2*j] + f1[2*j]);                                               
		//cout << "F0 number 0: " << f0[0][0] <<" " << f0[0][1] <<" " <<  f0[0][2] << endl;
		//cout << "Gravity: " << f0[1][0] << " "<<  f0[1][1] << " " << f0[1][2] << endl;
		Particle * p2 = new Particle(state[j]->mass);
		p2->position = newVec1;
		p2->velocity = state[j]->velocity+h/2.*(f0[2*j+1] + f1[2*j+1]);    
